...
If your sbt project is in a subdirectory of the repository rather than the root, pass the
`WORKING_DIRECTORY` input to the reusable workflow:
...
jobs:
  release:
    ...
    with:
      WORKING_DIRECTORY: ./worker-interface/scala
    secrets:
...
The `.tool-versions` file should be placed in the specified subdirectory alongside the sbt project.

## Java version

[Example `.tool-versions`](https://github.com/guardian/etag-caching/blob/main/.tool-versions)

Your repository _must_ contain an [`asdf`](https://asdf-vm.com/)-formatted `.tool-versions` file
...
match the _major_ version of Java specified in the file (eg `21`), and will _always_ use the
...
- **Artifact-producing modules** - any module (project or subproject) that creates an artifact for Maven Central
  ...
  - `licenses := Seq(License.Apache2)` - or whatever license you're using. Specifying a license is
    [_required_](https://central.sonatype.org/publish/requirements/#license-information) for submitting artifacts
    to Maven Central.
